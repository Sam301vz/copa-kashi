...
5 equipos, 3 intentos cada uno, **dos mesas simultáneas**, árbitros anotando desde tablets, HUD para transmisión
...
| `referee.html?mesa=A&juez=1` (hasta `juez=3`) | Jueces (tablet) | Hoja digital compartida por mesa; el juez 1 envía |
...
Ranking según el reglamento oficial: **mejor puntaje de los 3 intentos**; desempate con el **2.º** y luego el **3.º** mejor.
...
5. Pestaña **Usuarios → Agregar usuario**: crea la cuenta del staff (ej. `staff@copakashi.com`). Esa cuenta la usan control y árbitros.
...
- **Árbitro mesa A / B** → ábrelos en cada tablet e inicia sesión con la cuenta del staff.
...
1. **Ajustes**: nombre del evento, **jueces por mesa (1 a 3)** y los 5 equipos. Se genera el orden: 8 partidos, dos mesas a la vez.
2. **En vivo → Siguiente partido**: las hojas de los jueces se limpian y muestran el equipo e intento.
...
4. **Jueces**: hasta 3 por mesa, todos sobre **la misma hoja en tiempo real**. El **juez 1 es el principal** y es el único que envía o corrige. Cada tablet muestra quién está conectado. La primera vez aparece una guía de 7 pasos (botón **Guía** para verla otra vez).
...
7. Correcciones: el juez principal pulsa **Corregir puntaje**, o en **Resultados** borras el intento con ✕. Si una tablet falla: **Guardar sin la tablet**.
...
La app usa **5** (el texto manda sobre la hoja). Si tu árbitro principal decide otra cosa, cambia `"pointsEach"` de `m14_dest`.

El **bonus de M02** (escenas iguales en ambas mesas) lo marca cada árbitro; la hoja le muestra en vivo qué color marcó la otra mesa.
